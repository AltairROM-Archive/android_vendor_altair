...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ALTAIR_ROOT"/vendor/altair/build/tools/smali/baksmali.jar
        export SMALIJAR="$ALTAIR_ROOT"/vendor/altair/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ALTAIR_ROOT"/vendor/altair/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ALTAIR_ROOT"/vendor/altair/build/tools/"$HOST"/compact_dex_converter
    fi
